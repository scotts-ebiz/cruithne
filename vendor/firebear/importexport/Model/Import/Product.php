...
class Product extends MagentoProduct
{
    ...
    protected $manager;
    /**
     * @var \Firebear\ImportExport\Model\Import\Product\Integration\MageArrayMarketplace
     */
    ...
    /**
     * @var \Firebear\ImportExport\Api\UrlKeyManagerInterface
     */
    ...
    /** @var \Firebear\ImportExport\Model\Import\Product\Integration\WebkulMarketplace */
     ...
     * @param \Firebear\ImportExport\Model\Import\Product\Integration\WebkulMarketplace $webkulMarketplace
     * @param \Firebear\ImportExport\Model\Import\Product\Integration\MageArrayMarketplace $mageArrayMarketplace
    ...
    protected function saveProductsData()
    {
        ...
        if ($this->manager->isEnabled('Webkul_Marketplace')) {
            ...
            if (!$this->helper->getProcessor()->inConsole) {
                $verbosity = \Symfony\Component\Console\Output\ConsoleOutput::VERBOSITY_VERBOSE;
            }
    ...
    protected function saveProducts()
    {
        ...
        while ($nextBunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            foreach ($nextBunch as $rowNum => $rowData) {
                ...
                if (!isset($oldSkus[strtolower($this->getCorrectSkuAsPerLength($rowData))])) {
                ...
                if ($this->_parameters['generate_url'] && isset($rowData[self::COL_NAME])) {
                    ...
                    $isDuplicate = $this->isDuplicateUrlKey($urlKey, $rowData[self::COL_SKU], $storeIds);
                    if ($isDuplicate || $this->urlKeyManager->isUrlKeyExist($urlKey)) {
                        $urlKey = $this->productUrl->formatUrlKey(
                            $rowData[self::COL_NAME] . '-' . $rowData[self::COL_SKU]
                        );
                    }
                    $rowData[self::URL_KEY] = $urlKey;
                    $this->urlKeyManager->addUrlKeys($urlKey);
                }

                if (isset($rowData['remove_images']) && $rowData['remove_images'] == 1) {
                    if (isset($existingImages[$this->getCorrectSkuAsPerLength($rowData)])) {
                    ...
                    if ($this->_parameters['configurable_create'] && !$collection->getSize()) {
                        try {
                            ...
                            if ($this->_parameters['generate_url'] && isset($data[self::COL_NAME])) {
                                $storeIds = $this->getStoreIds();
                                $urlKey = isset($data[self::URL_KEY])
                                    ? $this->productUrl->formatUrlKey($data[self::URL_KEY])
                                    : $this->productUrl->formatUrlKey($data[self::COL_NAME]);
                                $isDuplicate = $this->isDuplicateUrlKey($urlKey, $data[self::COL_SKU], $storeIds);
                                if ($isDuplicate || $this->urlKeyManager->isUrlKeyExist($urlKey)) {
                                    $urlKey = $this->productUrl->formatUrlKey(
                                        $data[self::COL_NAME] . '-' . $data[self::COL_SKU]
                                    );
                                }
                                $this->urlKeyManager->addUrlKeys($urlKey);
