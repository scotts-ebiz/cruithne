...
read -p "Enter File To Copy: " COPY_TO_FOLDER
#echo $COPY_FROM_FOLDER

# Get the file name and location
read -p "Enter Location to Copy: " COPY_FROM_FOLDER
#echo $COPY_FROM_FOLDER
#echo $S3_BUCKET/$COPY_FROM_FOLDER $COPY_TO_FOLDER

# Run the command to list the files in the S3 bucket
aws s3 cp $COPY_TO_FOLDER $S3_BUCKET/$COPY_FROM_FOLDER --recursive --profile produser
