 ...
 */
namespace SMG\Vantiv\Gateway\Common\Builder;
 ...
 * @api
 */
class EnhancedDataBuilder extends \Vantiv\Payment\Gateway\Common\Builder\EnhancedDataBuilder
{
    /**
     * Max items count
     *
    ...
    public function build(array $subject)
    {
        ...
        if (in_array($ccType, self::ALLOWED_CREDIT_CARD_TYPES) && $order->getTaxAmount() > 0) {
            ...
            if ($shippingAddress) {
				...
				$postcode = substr($shippingAddress->getPostcode(), 0, 5);
    ...
    public function extract(array $subject)
    {
        ...
        if (in_array($ccType, self::ALLOWED_CREDIT_CARD_TYPES) && $order->getTaxAmount() > 0) {
            ...
            if ($shippingAddress) {
				$postcode = substr($shippingAddress->getPostcode(), 0, 5);
                $data['destinationPostalCode'] = $postcode;
