...
AdminRma.prototype = {
    ...
    addOrderItemToGrid: function (orderItem, className, hasUserAttributes) {
        var keepGoing = true;
        if (this.newRmaItemId > 0)
        {
            // loop through the number of rows to make sure that there are not duplicates
            for (i = 0; i < this.newRmaItemId; i++)
            {
                // get the old item id to pull information from the row
                var oldRmaItemId = "new" + i;

                // get the element from the page to get the element value
                var oldItemId_el = document.getElementsByName('items[' + oldRmaItemId + '][order_item_id]');

                // there should only be one because we are using a unique name
                // but we will check to make sure
                if (oldItemId_el.length > 0) {
                    var oldItemId = oldItemId_el[0].value;

                    if (oldItemId == orderItem['item_id']) {
                        keepGoing = false;
                    }
                }
            }
